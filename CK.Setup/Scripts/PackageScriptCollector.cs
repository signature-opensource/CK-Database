...
namespace CK.Setup
{
    public class PackageScriptCollector
    {
        ...
        Predicate<string> _availableScriptType;

        public PackageScriptCollector( Predicate<string> availableScriptType )
        {
            ...
            _availableScriptType = availableScriptType;
        }
        ...
        public PackageScriptSet Add( ISetupScript script, IActivityLogger logger )
        {
            ...
            PackageScriptSet s = _scripts.GetOrSet( script.Name.ContainerFullName, n => new PackageScriptSet( n ) );
            if( s.PackageFullName != script.Name.ContainerFullName )
            {
                logger.Warn( "Script '{0}' can not be associated to '{1}' (names are case-sensitive). It is ignored.", script.Name.FileName, s.PackageFullName );
                return null;
            }
            if( !_scriptTypes.Contains( script.ScriptType ) )
            {
                if( _availableScriptType != null && !_availableScriptType( script.ScriptType ) )
                {
                    logger.Warn( "Script type '{2}' is not available. Script '{0}' for '{1}' will be ignored.", script.Name.FileName, s.PackageFullName, script.ScriptType );
                    return null;
                }
            ...
            if( !s.Add( script ) )
            {
                logger.Warn( "Script '{0}' in '{1}' for {2} is already registered. It is ignored.", script.Name.FileName, script.Name.ExtraPath, s.PackageFullName );
                return null;
            }
            return s;
        }
