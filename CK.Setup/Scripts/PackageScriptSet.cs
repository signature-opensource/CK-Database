...
namespace CK.Setup
{
    public class PackageScriptSet
    {
        ...
        class CompareScript : IEqualityComparer<ISetupScript>
        {
            ...
            {
                Debug.Assert( xs.Name.ContainerFullName == ys.Name.ContainerFullName, "Internal use only, we are working on the same Container: names match." );
        ...
        /// <returns>True if the script has been added, false if the script already exists (see <see cref="SetupScriptExtension.GetScriptKey"/> for script identity).</returns>
        internal bool Add( ISetupScript script )
        {
            if( PackageFullName != script.Name.ContainerFullName )
            {
                throw new ArgumentException( String.Format( "Script '{0}' can not be associated to '{1}' (names are case-sensitive).", script.Name.ContainerFullName, PackageFullName ) );
            }
            return _scripts.Add( script );
        }

        public IEnumerable<string> ScriptTypes
        {
            get { return _scripts.Select( s => s.ScriptType ).Distinct(); }
        }

        public PackageTypedScriptVector GetScriptVector( string scriptType, SetupCallContainerStep step, Version from, Version to )
        {
            ...
            if( from == null )
            {
                if( versionStep.Any() )
                {
                    ...
                    return new PackageTypedScriptVector( maxVersion, noVersion );
                }
                return new PackageTypedScriptVector();
            }
            var scripts = versionStep.Where( s => s.Name.BelongsToUpgradeFrom( from ) ).ToList();
            if( scripts.Count == 0 ) return new PackageTypedScriptVector();
            if( scripts.Count == 1 ) return new PackageTypedScriptVector( scripts[0], noVersion );
            ...
            return new PackageTypedScriptVector( coveringScripts, noVersion );
        }
