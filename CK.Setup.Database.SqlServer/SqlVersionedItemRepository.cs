...
namespace CK.Setup.Database.SqlServer
{
    public class SqlVersionedItemRepository : IVersionedItemRepository, IDisposable
    {
        ...
        public SqlVersionedItemRepository( IDatabaseExecutor db )
        {
        ...
        public VersionedName GetCurrent( IVersionedItem i )
        {
            ...
            _db.Connection.ExecuteNonQuery( _get );
            Version v;
        ...
        public void SetCurrent( IVersionedItem i )
        {
            ...
            _set.Parameters[2].Value = i.Version;
            _db.Connection.ExecuteNonQuery( _set );
        }
        ...
        public void Delete( string fullName )
        {
            ...
            _db.Connection.ExecuteNonQuery( _del );
        }
        ...
        void Initialize()
        {
            if( !_db.ExecuteScriptNoLog( _script ) )
            {
...
AS
begin
	set nocount on declare @error int, @ret int, @rc int set @error = 0 begin tran --[!beginsp]
	set @ItemType = Upper(@ItemType);
	...
	begin
		...
		begin
			...
			set @error = @@Error if @error <> 0 goto ErCall if @ret <> 0 goto Error --[!call]
		end 
		else
		begin
	...
	begin
		...
		set @error = @@Error if @error <> 0 goto Error --[!catch]			
	end
...
GO
declare @ThisVersion varchar(32) = '1.0.0'
...
begin
	...
	(
		...
		constraint CK_tSystem_Id check (Id = 1)
	);
end
GO
if not exists( select * from CKCore.tSystem where Id=1 )
begin
    insert into CKCore.tSystem(Id) values(1);
end
GO
