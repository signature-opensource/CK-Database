...
namespace CK.Setup
{
    partial class MutableItem
    {
        ...
        MutableAmbientProperty EnsureCachedAmbientProperty( IActivityLogger logger, StObjCollectorResult result, IStObjValueResolver dependencyResolver, Type propertyType, string name, MutableAmbientProperty alreadySolved = null )
        {
            ...
            // Only the first entry point in the cycle will cache a new MutableAmbientProperty( this, name ) in its cache. Any other call to the same cycle will lead to (and return) this (empty) marker.
            // The only other case where we return null is when the requested propertyType is not compatible witn an existing cached property with the same name.
